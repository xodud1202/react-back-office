...
import type { NewsCategoryOption, NewsPressOption } from '@/components/news/NewsSearchForm';
...
const NewsListPage = () => {
  ...
  /**
   * 현재 선택한 언론사/카테고리 기준으로 목록을 다시 조회합니다.
   * @param event 검색 제출 이벤트
   */
  const handleSearch = useCallback((event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    // 선택 카테고리가 있으면 기사 목록만 다시 조회합니다.
    if (selectedPressNo && selectedCategoryCd) {
      loadTopArticlesByCategory(selectedPressNo, selectedCategoryCd);
      return;
    }

    // 카테고리 선택 전이면 스냅샷을 다시 조회합니다.
    if (selectedPressNo) {
      loadNewsSnapshot({ pressId: selectedPressNo });
      return;
    }

    // 선택값이 없으면 기본 스냅샷을 다시 조회합니다.
    loadNewsSnapshot();
  }, [loadNewsSnapshot, loadTopArticlesByCategory, selectedCategoryCd, selectedPressNo]);

  /**
   * 뉴스 검색 조건을 기본 스냅샷 상태로 초기화합니다.
   */
  const handleSearchReset = useCallback(() => {
    // 기존 선택 상태를 비우고 기본 스냅샷을 다시 조회합니다.
    setSelectedPressNo('');
    setSelectedCategoryCd('');
    setCategoryOptions([]);
    setAllRowData([]);
  ...
  return (
    <>
      ...
      <NewsSearchForm
        ...
        selectedCategoryCd={selectedCategoryCd}
        onChangePressNo={handlePressChange}
        onChangeCategoryCd={handleCategoryChange}
